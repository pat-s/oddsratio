...
```{r}
calc.oddsratio.gam(model = fit.gam, data = dat, pred = "x4",
                   values = c("A", "B"))
```
...
```{r}
calc.oddsratio.gam(fit.gam, pred = "x2", percentage = 20,
                   slice = TRUE, data = dat, quietly = FALSE)
```

If we assign this function call to a variable, we get a nicely formatted data.frame output. 
To suppress the console output, we can set `quietly = TRUE`.

```{r}
df <- calc.oddsratio.gam(fit.gam, pred = "x2", percentage = 20,
                   slice = TRUE, data = dat, quietly = TRUE)
df
```
